...
class AuthRemote {
  ...
  Future<void> login(LoginModel user) async {
    try {
      ...
      if (code != null && code >= 200 && code < 300) {
        ...
        if (access != null) {
          final local = await StorageUtils.getInstance();
          final lclAcc = await local!.putString(LocalName.access, access);
  ...
  Future<void> signout() async {
    final local = await StorageUtils.getInstance();
    final access = await local!.remove(LocalName.access);
    final refresh = await local.remove(LocalName.refreshToken);
